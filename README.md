...
「2つの印が食い違っている」ように見えます。バブルは配置未確定なら回答枠へ追従しますが、AX枠や
囲みで既に配置済みなら動かず、回答対象を示す印だけが更新されます。
...
ただしこれは**カードだけの規則**です。印はリング → AX枠 → 回答枠と最新の解決結果へ必ず更新します。
AX枠や囲みでカードが既に配置済みなら、回答枠が別の場所へ移ってもカードは動かず、印だけが回答対象を
正直に示します。ユーザーのドラッグだけはカード配置の例外で、その位置は次に指すまで勝ちます。
...
演出（走査線・重力・撮り直しのキュー）、通していない案内の経路 — と、今週の修正が署名ビルドでも
