...
**紫（`iris`）は「ここ・これ・あなたが触ったもの」専用**で、状態（処理中・注意）には貸しません。
マゼンタは観客がモデルで、実インターフェースの色に無いことが選定理由です。だから**画面上の印と
焼き込みは似せてはいけません** — 揃えると、モデルへの合図と人への合図が同じ意味に見えます。

**未統一が1件**: Copilotが実画面へ出す枠（`HighlightOverlayPresenter`）は`systemRed`のままです。
