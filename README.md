...
**バブルが動く理由は3つだけです。枠が決まった時、回答が別の物を指した時、ユーザーが動かした時。**
**そして、読み始めた後は動きません。** 回答が指す対象は最後の検証済みオブジェクトからしか読めないので、
本文が流れるターンでは1行目の数秒後に届きます。そこで動かすと、読んでいる文が手元から動きます。
印は正しい場所へ移り、カードだけがその場に残ります。
