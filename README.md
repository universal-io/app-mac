...
- 履歴・下書き・メモリはSupabase user IDごとに分離する。ログアウト時はローカルDBを閉じ、
  ...
  セッションにだけ一度移行する。
- Supabaseのログインセッションは、ACL方式の旧loginキーチェーンではなく**データ保護キーチェーン**
  （`kSecUseDataProtectionKeychain`）へ保存する。データ保護キーチェーンはコード署名ACLではなく
  アプリのアイデンティティで認可されるため、リビルドや別署名（ad-hocの検証ビルド等）で「loginキー
  チェーンへの保存許可」パネルが繰り返し出ることがない。テスト起動ではKeychainを開かない。旧login
  キーチェーンに残っていたセッションは移行せず、本変更適用後に一度だけ再ログインが必要。
...
- 右 Shift 2回: パネル表示、Vision開始、またはパネルを閉じる（閉→コンポーズ→Vision→閉）
- 右 Shift 長押し: 音声入力
- Esc: パネルを閉じる
