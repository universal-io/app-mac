...
- **解約は期間終了時に効きます。** 支払った月は使えるという扱いで、即時停止も日割り返金もしません。
  ...
  取り消せば表示も消えます。
- **ポータルから戻った時に1度だけ再取得します。** 契約はアプリが見えないWeb上で変わるため、
  戻ってきても古いプランを表示し続けると、ユーザーは解約が失敗したと判断します。起動のたびに
  取得はしません（このアプリはホットキーで何度も前面化するので、通常利用の前に通信を挟みません）。
  webhookの適用は1〜2秒かかり、すぐ戻ってきたユーザーはその競争に勝ててしまうため、
  戻った直後と数秒後の2回読みます。
