...
先回り文案は、画面とAccessibility由来の周辺文脈を低推論で読み、表示中の会話や作業状態から
ユーザーが次に入力する自然な文章を1案生成します。共通の判断指示とユーザーPersonaを別レイヤーで
渡し、現行の既定Personaは事業開発とソフトウェア開発を横断するFounder／Engineer／Designer／
Business professionalです。文案生成前に、最新メッセージの話者・宛先・行為主体を確認し、
相手の送付や作成をユーザー自身の行為として言い換えません。アプリ識別情報に応じて独立した
...
文案生成はmedium推論で、最新の送信者、宛先、ユーザーの立場、添付所有者、依頼、返信意図を
構造化してから本文を作ります。検証中はこの認知結果を文案下の説明に表示します。
明示的にパネルを閉じた時は呼び出し元アプリへフォーカスを戻し、
次回起動のCompose／Vision判定がUniversal I/O自身を前面アプリとして誤認しないようにします。

Composeは通常コンパクトに開き、画面分析中／レビュー中は操作列のスピナーだけで進行を示します。
文案、レビュー結果、または表示すべきエラーが届いた時にだけ下部を拡張します。Compose原文、
AI文案、レビュー文案、Vision質問の送信操作は、同じ紙飛行機アイコン、Enter操作、VoiceOver上の
用途別ラベルに統一します。AI文案には破棄操作を置かず、使わない場合はそのまま無視できます。
パネルは起動のたびにカーソルがある画面の中央へ配置し、表示後は背景や余白をドラッグして自由に
移動できます。前回の位置は保存しません。
