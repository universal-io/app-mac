...
| **格子** | `WashStyle.lattice*` | 幕の上の白い点。25pt間隔・4pt径・白16%。「読み取っている」を言う層。1点ずつ描く（タイルではない） |
| ├ **重力** | `WashStyle.gravity*` | 格子がカーソルへ寄る歪み。中心ほど強く、`1400pt`で0へ2乗で減衰。中心は35%寄る（密度約2.4倍）。芯はスポットライトで消えているので、見えるのは周囲の傾き。Reduce Motionでは無し |
| **走査線** | `WashStyle.sweep` | 幕の上を上から下へ1回走る淡い光の帯（160pt・白22%・0.32秒）。幕の入場と、案内の撮り直しで同じもの。Reduce Motionでは無し |
