...
| **格子** | `WashStyle.lattice*` | 幕の上の白い点。30pt間隔・4pt径・白16%。「読み取っている」を言う層。1点ずつ描く（タイルではない） |
...
| **走査線** | `WashStyle.sweep` | 幕が**下から上へ、光の前縁の後ろから現れる**（マスクで露出。前縁の光120pt・白22%、0.42秒、加速）。幕の入場と、案内の撮り直しで同じもの。Reduce Motionでは無し |
