...
1. **実機確認。Composeのバブルは一度も実機で見ていません**（[HANDOFF](HANDOFF.md)に見るもの6点）。
   R14/R15にも実機未確認の演出（走査線・重力・撮り直しのキュー）と、通していない案内の経路があります。
2. **D7**（24時間以上連続稼働したプロセスでgolden pathを通す）。起動直後だけを確認する
   チェックリストでは、R11の動機になった無音停止は原理的に検出できません。**このゲートは実際に
   欠陥を捕まえた実績があります** — build `7` は32時間50分稼働したプロセスが `uptime=0h0m` を
   出力したため公開しませんでした。

両方を通してから `bash tools/release.sh --promote 0.2.3 9` で公開ダウンロードを切り替え、
