...
### R14: 実画面の上で指して聞くVision（要件確定・実装未着手）

Visionを「スクリーンショットのパネルを出す」から「**モードに入る**」へ置き換えます。右Shift 2回で
実画面の上に覆いが乗り、入った瞬間に現在の画面の解説が始まります（質問は要りません）。解説は右下の
バブルに1文字ずつ出ます。**実画面のどこかをクリックすると、バブルがその場所の隣へ移り、そこについて
の解説が始まります。** そのまま続けて質問も打てます。Composeから右Shift 2回で入る経路も同じ形で、
現在の範囲選択の確認待ちは入口から外します。
...
指した場所は、`pointer`（画像内の正規化座標）と、送信画像へ焼き込んだ印の両方でモデルへ伝えます。
**覆いに描いた印は撮影から除外されるため（撮影は自アプリを除外しています）、焼き込みは選択肢では
なく必然です。** AXの実測矩形はバブルの置き場所と枠に使い、モデルの根拠にはしません（Gatewayは
候補の矩形をモデルへ渡しません）。ホバーでは呼び出しません — 1リクエストが1ユニットを消費するため、
カーソルが通過するたびのモデル呼び出しは成立しません。
