...
- Vision: **実画面の上のモード**。右Shift 2回で覆いが乗り、入った瞬間に現在の画面の解説が
  ...
  **バブルの中はメッセンジャーと同じスレッド**（AIは左でアプリアイコン、ユーザーは右で本人のアバター、
  ...
  見えているものとモデルが覚えているものは常に一致する（実装済み、実機確認待ち 2026-09-03。正本は
...
確認後に `bash tools/release.sh --promote 0.2.4 11` で公開ダウンロードを切り替え、
そのソースコミットへ `v0.2.4` タグを付けます。**候補は再ビルドしません。**
...
build `10` の確認で見るのは、この間の実機確認で未消化のもの — Composeのバブル6点、通していない案内の
経路 — と、今週の修正が署名ビルドでも同じに動くことのスモーク。演出のうち走査線は入場だけになり
（撮り直しの合図は2026-09-07に撤去）、案内ループの7件の修正は2026-09-08にDebugで確認済みである。quotaバナー・メーター・×・使い方・選択バブル・配置はDebugビルドで
確認済みである。
...
`0.2.3`は`GatewayAvailability`（立ち続ける利用不能表示）と配置状態機械で条件に軽く触れるが、
どちらも起動直後から検証でき、長時間後にだけ壊れる性質ではないため、今回は実施しない
（オーナー承認 2026-09-02）。

確認後に `bash tools/release.sh --promote 0.2.3 10` で公開ダウンロードを切り替え、
