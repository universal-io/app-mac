...
リング半径・ビートの距離と長さがここにあり、指差しの覆いも、Copilotが別ウインドウで出す枠も、
...
**「案内を開始」（Copilotの入口）は、ユーザーが質問を打ち込んだ時に出ます。** 語句では判定しません
（`VisionSession.offersGuidance`）。指差しだけのターンには出しません — 指した人が求めているのは
それについての答えであって、案内を始める意図は持っておらず、そのターンの本文「ここについて」が
そのままCopilotの目的になってしまうためです。Chrome等がクリック対象のAccessibility矩形を返さない
場合も入口は表示し、矩形がある時だけ対象をハイライトします。矩形が無い時は文章案内を継続して
ユーザーのクリック後に画面を再評価します。

以前はキーワード26語の一致で判定していましたが、「どうやって」があって「どうすれば」が無いなど、
語を足す作業に終わりがなく、要望（迷ったら出す方に傾ける）と逆を向いていたため廃止しました。
「これは何ですか?」にも入口が出るのは、この方針で受け入れているコストです。
