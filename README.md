...
- Compose: 文章の作成、音声入力、レビュー、対象アプリへの送信。**Visionと同じコンパニオン
  ...
  （`0.2.3` build `10` の署名ビルドで実機確認済み、2026-09-03）。
...
- Vision: **実画面の上のモード**。右Shift 2回で覆いが乗り、入った瞬間に現在の画面の解説が
  ...
  **バブルの中はメッセンジャーと同じスレッド**（AIは左でアプリアイコン、ユーザーは右で本人のアバター、
  最新が下）。1つの主題の中の質問と回答は下へ積まれ、別の場所を指すと新しいスレッドが空から始まる —
  見えているものとモデルが覚えているものは常に一致する（`0.2.4`の署名ビルドで実機確認済み。正本は
  [マスタープラン R17](docs/universal-io-master-plan.md)）。
  選択テキストがある時は、その内容が回答対象になる。
  静止画パネル（縮小したスクリーンショット、拡大縮小、選択カード）はR14で撤去した。
  モデル、Gateway、AX収集、captureの開発情報はバブルのツール名横の情報ボタンへ畳み、
  まとめてコピーできる。
  案内の撮り直しは無音で行う。撮影が確定した瞬間に撮影範囲へ幕を一瞬（走査線1本分）戻す合図
  （2026-08-26）は、幕の無い実画面の上では画面の不具合にしか見えなかったため2026-09-07に撤去した。
  考え中はチップの脈動が示す。「画面を読んだ」合図をやり直すなら設計から
- 案内（Copilot）: 打ち込んだ質問の回答が次の一手なら幕が引き、実画面の上で次に押す対象を枠で示す。
  操作後に画面を再取得し、目的に到達するまで続く。バブルは解説と同じ1つで、チップが状態を名乗る。
  **撮り直すのは「ユーザーが進んだ」とイベントが言う時だけ**（2026-09-03、正本は`GuidanceTrigger`）:
  クリックは撮り直し、**文字入力欄へのクリックは撮らず、最初のキーが打たれてから入力が約0.8秒止まった時に撮る**
  （欄ごとに1手が走らない。打たずに離れた欄は操作に数えない — エディタのカーソル移動で毎回撮った2026-09-06の修正）、
  **スクロールは撮らず枠だけが要素に追従する**（AXの位置を読み直す。要素が消えれば枠も消える。バブルの上のスクロールは数えない）。
  評価中に次の操作が来たら、撮影前なら吸収し、撮影後なら**走っている評価を打ち切って最新の画面から**やり直す
  （以前は捨てていて、消えたボタンの位置に枠が残った）。キー入力は「あったこと」だけを見て内容は読まない
- 履歴: 実際に送信した内容のローカル履歴

## 実画面の上のVision（R14・実装済み、`0.2.3`で公開済み）
...
引っ込み枠になります。同時に2つ描くと、ユーザーには「2つの印が食い違っている」ように見えます。

**回答の枠は、ジェスチャーが場所を確定できなかった時の妥協策です**（2026-09-03）。囲み、またはAXが
実測できたクリックでは、場所は問い合わせる前に分かっているので、印は軌跡／AX枠のまま動きません。
回答が返す`targetCandidateId`は候補の**ラベル文字列**からモデルが選ぶもので（矩形は渡していない）、
自分の文中の語に合うラベルへ飛ぶことがあります — 実機では、囲んだ2つのアイコンを正しく解説しながら、
枠だけが文中に出た「デバッグナビゲータ」ボタンへ移りました。解説と枠が別の根拠で決まる経路はここだけ
なので、**AX実測ありでは実測要素と重なる回答枠だけを採用し、囲みでは回答枠を採用しません**
（`vision.result highlight=gestureKept`で回数を記録）。AXが何も測れないクリックだけは、モデルの箱が
「ここだと理解した」を示す唯一の手段なので、従来どおり回答枠が印になります。案内の枠（次に押すもの）は
別の仕事で、この規則の対象外です。

**1回の指差しが持つ自動配置は1回だけです。** 状態は
`resolving → committed → contentVisible`で、AX実測要素またはユーザーが囲んだ範囲はGatewayへ送る前に
`committed`になります。AXヒットが無いクリックだけは`resolving`の間SSE本文を画面へ出さず、最後の
検証済み回答から枠（candidateまたはannotation）を同期反映してから本文を表示します。枠が無ければ
`unavailable`として現在位置のまま本文を出します。**文章が見えた後に自動で動く経路はありません。**
ただしこれは**カードだけの規則**です。印はAXヒット無しのクリックでだけ回答枠へ更新します（上記）。
ユーザーのドラッグだけはカード配置の例外で、その位置は次に指すまで勝ちます。

この順序を守るため、AXヒット無しの実測例では初文字が約3.5秒から最終結果の約5.5秒へ遅くなります。
AX実測・囲み・通常質問のストリーミングは変わりません。位置と文章の矛盾を無くすために受け入れた
トレードオフで、変更はクライアント側だけです（Gatewayには手を入れていません）。
**リングは理由になりません** — リングは「クリックを受け取った」と言うだけで、対象がどこにあるかは
言っていないからです。だから**クリックした点は置き場所の基準になりません**（`BubbleAnchor`に
点を持つケース自体がありません）。

セッションが始まると、バブルは画面の右下で待ちます。クリックするとリングがその場に出ますが、
**バブルは動きません**。AXが要素を実測して枠が確定した瞬間に、**右下から枠の隣へ1回だけ**移動します。
回答はその後で届くので、しばらく枠の隣で考え中の表示になります。AXが何も測れなかった時は
**どこへも動きません**（対象の場所が分かっていないのに「これの隣」と言うのは、根拠のない主張になる
ため）。回答が枠を返せば、**その枠へ移動してから回答全文を表示します**。

この規則は3回戻りました。1回目は`showRing`から配置呼び出しを外し、2回目はアンカーへの代入も外し、
3回目は実測frameをアンカーにした — どれも「クリック点を使わない」という約束で、約束は次の編集で
破れます。4回目に**点を持てる型そのものを無くしました**。
5回目の調査で、根本原因を「1ターン1配置が状態として存在しないこと」と確定し、上記の状態機械と
未解決時だけの本文バッファリングで構造として閉じました。印とカードを別の状態として扱う修正後、
実機でも意図した挙動を確認済みです（2026-09-02）。

**伸び縮みは移動ではありません** — 回答が届いて高さが変わっても置き場所は解き直さず、左上を保って
下へ伸びます（`VisionBubblePlacement.resized`）。**例外は対象の上に置かれたバブルだけで、下端を保って上へ伸びます**
（2026-09-08。下へ伸びると対象と、対象が開くメニューを覆うため）。隣に置く時は横の余地だけを見て、縦は画面に収まるまで
ずらします（GA4の画面下端の期間ボタンで、背の高いバブルが「上」へ送られて対象を覆った）。
...
| **格子** | `WashStyle.lattice*` | 幕の上の白い点。36pt間隔・4pt径・白16%。「読み取っている」を言う層。1点ずつ描く（タイルではない）。**動かない** — カーソルへ寄る歪み（重力）は2026-09-03に撤去した（安っぽく見えた） |
| **走査線** | `WashStyle.sweep` | 幕が**下から上へ、光の前縁の後ろから現れる**（マスクで露出。前縁の光120pt・白22%、0.42秒、強い加速 `(0.7, 0, 0.84, 0)`）。幕の入場だけ（案内の撮り直しの合図は2026-09-07に撤去）。Reduce Motionでは無し |
...
| **バブル** | `VisionBubbleView` | スレッド（`VisionThreadView`）と入力欄を持つコンパニオン。製品が喋る唯一の場所。**1スレッド＝1主題**で、別の場所を指すと空から始まる。**入力欄以外のどこを掴んでも動かせ、動かした位置は次に指すまで勝つ**。置き場所を決める値は`BubbleAnchor`だけで、**1回のジェスチャーで動くのは1回**（リングでは動かず、計測が済んでから隣へ来る） |
| **焼き込み** | `VisionPointerMark` | **送信画像のバイト列の中にだけ**あるマゼンタのリング＋十字。**画面には出ない** |
| **解説** | `AppMode.vision` | 幕が乗っている状態。クリックは質問。バブル左上のチップが名乗る |
| **案内** | `AppMode.copilot` | 幕が無い状態。クリックは操作で、次に押す対象に枠が出る。同じセッション・同じバブル。チップは「案内中」と名乗りアイコンが脈打つ。×で解説へ戻る |
...
- **ブラウザの窓では、AXで測った要素のうちページの中にあるものだけを送る**（2026-09-03）。
  ブックマーク・タブ・拡張機能・アドレスバーは送らない。ブックマークのラベルはURLそのもの、
  タブのラベルは他サイトのページ名なので、見ている画面のアドレスを伏せながら他サイトのアドレスと
  メールアドレスを毎ターン送っていた。GA4実測で毎画面57件、候補の47%がこれだった。
  webアプリ（Electron）は全体がページの中なので何も減らず、ネイティブアプリはページを持たないので
  従来どおり全要素を送る。判定は`VisionCandidateScope`。
...
**公開ダウンロードは `v0.2.4`（build `11`、バイナリソース `6ea0cc2`）です。** 2026-09-13に公開しました。
...
Universal binary（x86_64 + arm64）、本番Gateway（`https://api.universal-io.com`）向き、
公開URLからの再取得でSHA-256が一致することを確認済みです。221 unit testが通り、タグは`v0.2.4`です。

2026-09-06にオーナーの実機で見つかった案内の欠陥7件の修正（枠が出ない・赤い「Canceled」・撮影が連続）が
公開版へ入った最初の版です。R17（バブルのメッセンジャー型スレッド）、ブラウザの窓でページの中の要素だけを
送る候補スコープ、バブルの掴み手と伸びる向きの修正も同じ版に含みます。

署名ビルドでのスモークは2026-09-13に実施し、案内ループ・R17スレッド・掴み手・2画面の撮影対象は
オーナーが実機で確認しました。候補スコープは実機のunified logで確定しています —
`vision.axCollected candidates=121 chrome=58 webArea=true`（GA4の「ページとスクリーン」）。
**送ったのはページの中の121件で、Chrome自身の部品58件は落ちています**（2026-09-03の実測57件とほぼ同じ）。
同セッションの記録577件に`failed`・`cancel`・`denied`は無く、案内は`mode=guide highlight=resolved`で
枠を解決し、`guide.act.superseded`（走っている評価を打ち切って最新の画面からやり直す）も働いていました。

**D7（24時間以上連続稼働したプロセスでのgolden path）は実施していません**（オーナー判断 2026-09-12）。
`GuidanceTrigger`の監視の設置と解除、入力が止まってから撮るまでの0.8秒のタイマー、評価の世代管理は、
[manual-golden-paths.md](docs/manual-golden-paths.md)の再実施条件「セッションの生存・再入・タイマー・
キャッシュ・監視の設置と解除に触れた」に当たります。**条件に当たったまま公開する**ことを記録して
おきます — 公開の間隔を1日以上かかる試験で決めない、という2026-08-31の判断を優先しました。
稼働時間に関係する不具合の報告を受けたら、その修正では省かないこと。

前版の `v0.2.3`（build `10`、バイナリソース `db1feba`）は2026-09-03に公開し、
`https://dl.universal-io.com/releases/0.2.3/build-10/Universal-IO.dmg`、SHA-256
`fe1f1796737ec95cc6c90462d120e70f22ebf14070673f3c76e6f96dd644c7e8`として不変保存されています。
...
枠切れを「もう一度お試しください」と案内していた文言、試す前に言う利用不能バナー、選択起動で覆いを
出さない提示、バブル配置の1ターン1配置（印とカードの分離まで） — が候補ソースの後に十数コミット
入った。**検証してきたコードと公開するバイナリが別物**になったため、規則（候補は再ビルドしない・
署名ビルドごとにbuild番号を上げる）に従い、build `9` は公開ダウンロードへ向けないまま終える
（build `6`・`7`と同じ扱い。DMGは記録として`dist/`に残す）。

`0.2.3`でもD7は実施していない。`GatewayAvailability`（立ち続ける利用不能表示）と配置状態機械で
再実施条件に軽く触れるが、どちらも起動直後から検証でき、長時間後にだけ壊れる性質ではないため
（オーナー承認 2026-09-02）。D7を毎リリースの必須ゲートから外したのは2026-08-31のオーナー判断で、
1回に1日以上かかり、公開の間隔がその長さで決まってしまうためである。
