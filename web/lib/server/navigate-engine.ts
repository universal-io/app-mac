...
async function planTask(args: {
  ...
  if (!response.ok) return EMPTY_PLANNER_EXECUTION;
...
function resolveRoleRoute(
  ...
  for (const [vendor, modelId] of [
    [preferredVendor, preferredModelId],
    [fallbackVendor, fallbackModelId],
  ]) {
    const apiKey = apiKeyFor(vendor, env);
    const endpoint = VENDOR_ENDPOINTS[vendor];
    if (apiKey && endpoint) return { vendor, modelId, apiKey, endpoint };
  }
  return null;
}
