...
function requestBody(input: ScreenUnderstandingEngineInput): Record<string, unknown> {
  ...
  const task = input.guidance
    ? `Continue one human-guided task using this newly captured screen. The user has acted since the previous capture. Goal: ${input.guidance.goal}\nPrevious instruction: ${input.guidance.previousInstruction}\nDecide from the new screenshot whether the goal's exact requested information is now visible. Use answer mode only when the screen shows the specific breakdown, dimension, or metric the goal asked for; then state that result with a null target. A related but different view is NOT completion: e.g. a browser breakdown does not satisfy a request for a device breakdown, a weekly total does not satisfy a request for a daily one. When the exact requested view is not yet shown, use guide mode for exactly one next action toward it — including changing a dimension/segment selector, opening a tab, or switching a report control that is visible on this screen — and return the matching supplied target ID when one exists. Only conclude the goal is unreachable, with a clarification, after the visible controls offer no path to it; do not settle for a near-miss. Do not repeat the previous instruction when the screenshot shows it has already been completed.`
    : question
