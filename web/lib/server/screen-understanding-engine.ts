...
function requestBody(input: ScreenUnderstandingEngineInput): Record<string, unknown> {
  ...
  const task = input.guidance
    ? `Continue one human-guided task using this newly captured screen. The user has acted since the previous capture. Goal: ${input.guidance.goal}\nPrevious instruction: ${input.guidance.previousInstruction}\nDecide from the new screenshot whether what the goal asked for is actually shown now. Use answer mode only when this screen presents the specific thing the user requested; then state it with a null target. A screen that is similar or adjacent to the goal but not the exact thing requested is NOT completion — never report a near-miss as the answer. When the requested result is not yet shown but this screen exposes a control that could move toward it (any visible menu, tab, field, toggle, selector, or link), use guide mode for exactly one next action and return the matching supplied target ID when one exists. Conclude the goal is unreachable, with a clarification, only after the visible controls offer no path to it. Do not repeat the previous instruction when the screenshot shows it has already been completed.`
    : question
