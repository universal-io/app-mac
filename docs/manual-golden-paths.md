...
- [ ] 採用結果をクリップボードへコピーできる。
- [ ] 解説表示・返信案コピー後もTransformの選択文と結果が履歴へ追加されない。
...
- [ ] メモリカード削除後、ローカルと同期先のtombstoneに本文・相手名が残らない。
- [ ] AでログアウトしてBでログインしても、Aの履歴・下書き・メモリが表示・注入・同期されない。
- [ ] 別Macで同じメモリを同時編集すると競合が表示され、選んだ側だけが同期される。
- [ ] ネットワーク停止時はメモリ画面に同期失敗と再試行を表示する。
- [ ] アカウント画面のusageが成功リクエスト後に更新される。
- [ ] 使い捨てテストユーザーで退会確認をキャンセルでき、確定後は再ログイン不可かつMac内の
  履歴・下書き・メモリが消えている。有効契約中の退会は拒否される。
