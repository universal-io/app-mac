...
- [ ] 24時間以上連続稼働させた同一プロセスで、Compose（下書き→レビュー→送信）を通す。
- [ ] 同じプロセスでVisionを起動し、画面画像が表示され、初期説明が返る。
- [ ] 同じプロセスで追加質問と「案内を開始」（Copilot）が動く。
- [ ] 同じプロセスで右Shift長押しの音声入力が下書きへ入る。
- [ ] 上記の各操作が、成功でも失敗でも有限時間で終わる（スピナーのまま止まらない）。
- [ ] 管理画面「設定 › 動作記録」をコピーし、各操作に `vision.start` → `vision.request` →
      `vision.turn` が並んでいることと、uptimeが24時間以上になっていることを確認する。
- [ ] コピーした動作記録に、入力本文・AIの回答・ウインドウタイトル・ホスト名が含まれていない。
- [ ] マルチディスプレイ、スリープ復帰、ネットワーク切断→復帰をはさんでも上記が成立する。
- [ ] メニューバーの「Universal I/Oを再起動…」で確認ダイアログが出て、再起動後もログイン状態と
      保存済み下書きが残る。
