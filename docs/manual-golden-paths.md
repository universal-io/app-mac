...
- [ ] **バブルは入力欄以外のどこを掴んでも動かせる**（余白・チップ・読む面の空き・状態行）。答えが伸びても
...
- [ ] 打ち込んだ質問の回答が次の一手（`guide`）なら、幕が引いてチップが「案内」になり、次に押す対象に
  枠が残る。クリックは対象アプリへ通り、操作後に新しい画面を再評価して次の一手が出る。
- [ ] 回答が`answer`の時だけ「案内を開始」が出て、押すと幕が引き最初の一手を取りに行く。
- [ ] 案内チップの×で幕が戻り、会話はバブルに残る。目的到達後は幕を戻さず「解説を閉じる」で終わる。
