...
- [x] VS Codeの拡張webview（Claude Codeパネル）で文章を選択して右Shift×2すると、選択した内容
      ...
      主対象として読む経路で成立）。
- [ ] AXが選択文字列を返す画面（Chrome Gmail等）で`selection`カードが出て全文が扱われる。
      2026-08-01のprompt変更時点では`status: none`（AX取得が自前の探索条件で失敗していた）。
      祖先walkのweb areaスキップと走査上限を修正し、probeでは1msで227 units取得を確認済み。
      **実機での再確認が必要**（クライアント修正のため要再ビルド）。
