...
> 🔴 **仕様の優先順位**: 基盤作り直し（Phase 3〜4）の間、**アプリ挙動の仕様の正は
> 「フラグ OFF の旧経路コード」ただ一つ**。ドキュメントの挙動記述と食い違ったら
> コードが正しい（foundation-rebuild-plan.md §3 移植規律 6）。
...
| ドキュメント | 役割 | 状態 |
|---|---|---|
| [foundation-rebuild-plan.md](foundation-rebuild-plan.md) | **現行開発の正本**。基盤作り直し（シャーシ交換）の計画と進捗 | Phase 4 準備中 |
