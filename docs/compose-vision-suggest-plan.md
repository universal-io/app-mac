...
- Composeは初期表示から`680×660`とし、下部に「自動返信モード」と標準switchを常設する。
  オフでも領域は畳まず、オンへ戻した時は保持中の先読み画像から同一セッション内で生成を再開する。
  待機スピナーは自動返信領域内へ置き、設定は従来どおりUserDefaultsと設定画面で共有する。
- Visionの回答が`guide`なら、AX candidate矩形が取れなくても「案内を開始」を表示する。
  Copilotは文章案内だけでも開始し、対象矩形がある場合に限ってハイライトを追加する。
- 明示的な操作質問はローカルの語彙判定でもCopilot候補にし、モデルが`answer`と返しても導線を
  表示する。Vision promptも「どこで取得／どう開く／何をクリック」等は必ず`guide`と明記する。
