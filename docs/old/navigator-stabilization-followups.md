> **ARCHIVED (2026-07-15)** — 後継: [../copilot-challenge-3.md](../copilot-challenge-3.md)。
> §0.9〜0.9.1（ワンコール・ピボットの決定と受け入れ条件）は後継 §5 に吸収済み。
> §0.5〜0.8 の4役職/v3-v4並走/レシピ計画は第2次挑戦として総括された（後継 §2）。
> 当時のコード実態・計測手順の記録として保存する。

# Navigator / Copilot Accuracy Plan

最終更新: 2026-07-15 ／ ステータス: **進行中**（§0.9 ワンコール・アーキテクチャへピボット。
§0.6〜0.7 の4役職実行制御はユーザー経路としては打ち切り、計測資産のみ流用）
...
### 0.9 ワンコール・アーキテクチャへのピボット（2026-07-15、オーナー決定）🔴 現行方針

**本節が §0.6〜0.7 の実行制御方針を上書きする。** 4役職分割（Planner / Grounder /
Verifier / Renderer）＋抑止ゲートをユーザー経路として磨く方向は打ち切る。実画面計測で
確認した根拠: (1) v4 の汎用モデル判定（model_generic）はレシピなしで正しく verified を
返せた＝画面理解は成立している、(2) 一方で表示を握る v3 ラベル一致 grounding が同名
ラベルを構造的に取り違え、(3) fail-closed ゲートが正しい案内まで検閲し、(4) 役職分割の
接合部（snapshot 競合・ルーティング誤配線・初手指示の混入）が体験を壊し続けた。
モデルの能力ではなく、モデルを囲い込む配管が精度低下の主因だった。

新方針（15分プロトタイプの構造を正本にし、支柱を2本だけ足す）:

- **基本形**: 毎ターン、最高品質のマルチモーダルモデル**1回**の呼び出しに全部を渡す
  （十分な解像度の画像・十分な出力予算・タスク文脈・履歴の痩せを解消）。
  返答は「案内文＋対象要素」。
- **支柱1（座標はコードが持つ）**: AX/OCR 候補一覧（親文脈付き）を同じ1コールに渡し、
  モデルには candidate ID で選ばせる。座標変換・矩形描画はコード。同名ラベルは親文脈で
  構造的に区別される。別役職の Grounder は置かない。
- **支柱2（古い画面は機械で弾く）**: クリック後の画面安定検知（実装済み）で
  遷移中・stale の画面をモデルに見せない。
- **撤去**: safe_to_prompt によるハイライト拒否権、ユーザー向け技術警告（§0.8）、
  役職分割のユーザー経路への関与、[[loc]]/自由座標依存。
- **計測として残す**: trace/usage/eval、Run 署名と shadow 計測（体験に触らせない）、
  candidate 収集、遷移検知。レシピ/Pack は §0.7 どおり「後から載せる精度レイヤー」。
- **実装順**: ①新エンジンを白紙モジュールとして実装（旧 navigate-engine への継ぎ足し
  禁止）→ ②Copilot/Navigator を新エンジンに切替、ゲート撤去 → ③15分版と同条件の
  実画面比較（golden set）→ 合格で旧経路を削除。
- プロジェクト全体の作り直しはしない（ビッグバン失敗 → シャーシ交換成功の教訓、
  foundation-rebuild-plan.md）。書き直すのはナビゲーターの実行エンジンのみ。

ピボット直前の状態は tag `pre-one-call-pivot-20260715`（main `c19c5d0`）として保存。

#### 0.9.1 追補（2026-07-15夜、オーナー承認の詳細化）

本ピボットは「ハーネスを軽くする」のではない。**認知を単一コアへ集約し、堅牢性の置き場所を
LLM多段（独立でない検証＝同じ誤りの増幅）から決定論的外殻（証拠と整合性）へ移す。**
製品定義は自律操作エージェントではなく、画面を理解して人間に次の行動を案内する視覚コパイロット。

- **DOMは廃止しない**: Web=DOM由来候補（拡張なしの標準経路ではChrome AXのWebArea経由、
  拡張は精度アドオン）、macOS=AX、取得できない文字=OCR。ただしどれにも単独で正解を
  決めさせず、すべて親子関係・状態付きの「候補」として同じ1コールへ渡す。
- **構造化応答契約**: `mode(answer|guide|completed|clarification) / message /
  target_candidate_id / progress` の単一JSON。モデルに自由座標を推測させない。候補が
  取得できないCanvas等のみ、明示・記録される低信頼のvisual bbox経路を許す。
- **計画は固定しない**: 概略経路は説明してよいが、確定するのは毎ターン「次の一手」のみ。
  UI変更・権限差・想定外画面には毎ターン再計画で対応する。操作後の進捗判断は、別人格の
  Verifierによる評論ではなく、新しい画面（クリック後の新規capture）という独立証拠に
  基づく次ターンの判断として行う。
- **整合性検査とゲートの区別**: stale capture・capture scope不一致・存在しないcandidate ID・
  範囲外座標の拒否は**データ整合性検査**として残し強化する。モデルの正答を検閲する
  信頼度ゲート（safe_to_prompt棄却等）は置かない。
- **検証モードはfail-loud**: 品質上限測定の間、GPT-5.6 Sol（Responses API・十分な解像度・
  十分なreasoning/output予算）が使えなければ明示的に失敗させる。フォールバック・
  別モデル混入・サイレント経路切替は禁止。実効モデル/route/fallback発生は毎ターン
  記録し、開発ビルドでは表示する。
- **合格条件（新エンジンの受け入れ基準）**: ①全ターンの実効モデルがGPT-5.6 Sol、
  ②モデル出力の位置指定はcandidate IDのみ（低信頼bbox経路は件数を記録）、③同名ラベルを
  親文脈で区別できる、④stale/scope不一致の採用0、⑤fallback発生0（発生=テスト失敗）、
  ⑥全ターンがtraceから再生可能。成立確認後に初めて、画像縮小・履歴圧縮・軽量モデル・
  キャッシュで速度とコストを下げる（順序の逆転が今回最大の誤りだった）。
