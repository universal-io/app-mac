...
最終更新: 2026-09-03 ／ ステータス: `v0.2.3`（build `10`、タグ`v0.2.3`）を公開ダウンロードへ切替済み。R14・R15は実機で動作中（指した対象の認知精度100%、2026-08-24／案内が成立、2026-08-25）。R14のバブル配置は1ターン1配置の状態機械へ改修し、印とカードを独立させた挙動まで実機確認済み（2026-09-02）。**F12は閉じ、R14は完了。** Composeのバブルも同じ署名ビルドで実機確認済み（2026-09-03）。R12はE1（測定）進行中。**R16（道具が押す＝操作）は2026-08-30に設計を確定し、未着手。** **R17（バブルの対話スレッド）は2026-09-03に完了**（実装・同日の見た目修正・印の根拠の改訂・幕の重力撤去まで
実機確認済み）。200 unit test通過。**次はR8 M6（Skillsの拡充）。**
...
- **M6（進行中）** Salesforce / HubSpot / freee / SmartHR / GA4等を追加し、1ツールあたりの追加コストが
  ...
  出し分ける設計の実地検証になる。**2026-09-03、進め方を「GA4を1本目の完全サポートSkillにする」実証
  実験へ改めた**（用事一覧・器の論点・実測記録は[ga4-complete-skill-plan.md](ga4-complete-skill-plan.md)）。
  Salesforce等の横展開は、GA4で追加コストが定数だと示した後。
...
### R14 — 実画面の上で指して聞くVision（完了、2026-09-03。F1〜F12すべて実機で動作中）
...
2. **印とカードは別の状態機械。** 印はリング → AX枠 → 回答枠と進むが、**回答枠へ進むのはジェスチャーが
   場所を確定できなかった時だけ**（2026-09-03に改訂。下記「印の根拠」）。クリック点はカードの配置根拠にしない。
...
#### 印の根拠（2026-09-03。2026-09-02の「印は必ず回答枠へ更新する」を改訂）

**枠は妥協策である。** ユーザーが指した場所をアプリが理解できていれば、印はその場所に立っていればよく、
回答が枠を返す必要は無い。枠が要るのは、指された場所に何があるかをアプリが判別できないクリック
（本文・画像・グラフ、AXヒット無し）だけで、そこではモデルの箱が「ここだと理解した」を示す唯一の表現になる。
ゆえに**「ユーザーの位置は合っているのに枠が別の場所」は起きてはならない**。

**2026-09-03の実機で、それが起きた。** Xcodeのデバッグナビゲータで ⓘ と一時停止アイコンの2つを囲んだ
（wire dumpで軌跡の位置を確認、`vision.region drawn=219 kept=219 candidates=58`）。解説は囲んだ2つを
正しく説明したが、`vision.result highlight=resolved` で枠はナビゲータバーのデバッグナビゲータ（スプレー缶）
ボタンへ移った。**解説と枠は同じ回答の中の別の出力である**: `message`はモデルが画像の焼き込みを見て
書き、`targetCandidateId`は矩形を渡されていない候補一覧から**ラベル文字列**で選ぶ。文中に
「デバッグナビゲータ」と書いたので、そのラベルのボタンのidが添えられた。プロンプトは囲みを
「全体を主題にし1つを選ぶな」、対象idを「行動が要る時以外null」と定めており、返したこと自体が逸脱である。

**規則**（クライアントのみ。Gateway・プロンプトは無変更）

| ジェスチャー | アプリが知っていること | 回答枠 |
|---|---|---|
| 囲み | 囲んだ矩形 | **採用しない。** 軌跡が印のまま |
| クリック、AX実測あり | 要素の矩形 | **実測要素と重なる時だけ採用**（同じ場所か、その内側のより具体的なコントロール）。外ならAX枠のまま |
| クリック、AX無し | 点だけ | 従来どおりモデルの箱を印にする（妥協の場所） |
| 案内中 | — | 対象外（枠＝次に押すもの。別の仕事） |

追従質問（囲んだ後に打った質問）にも同じ束縛が効く。ジェスチャーの幾何は次に指すまで主題であり、
`VisionSession.gestureBound` が `pointedCandidate` / `pointer` から導く。棄却は
`vision.result highlight=gestureKept` に記録し、ラベル合わせの外れる頻度を測る（「ずれないようにする」議論の
材料）。実装は `VisionSession.answerHighlight(for:candidates:toleratingUnplaceableTarget:keeping:)`、
`VisionAnswerHighlightTests` 4件で固定。

**「文章を枠に従わせる」は採らない。** 枠の側が文章と別の根拠（ラベル合わせ）で決まっている以上、
正しい文章を間違った枠に合わせることになる。再問い合わせも要り、1ジェスチャーで2ユニット消費する。
...
- **AX候補は操作系13ロールだけで、ブラウザではページの中だけ**（`VisionObservationCaptureService.swift:145-149`、
  絞り込みの判定は`VisionCandidateScope`）。本文テキスト・
...
- **F11（完了）** 静止画パネル系を撤去した。実際の削減は約1,000行で、`ZoomableScreenshotView`
  ...
  - Copilotの「案内を開始」もバブルへ移した。**パネルは案内が始まった後にしか出ないため、
    ...
    のではなく、撤去の前から到達不能だった）。
- **F12（完了、2026-09-03）** build番号を`10`へ上げて署名ビルドし、`v0.2.3`として公開ダウンロードへ
  切り替えた（ソース`db1feba`、タグ`v0.2.3`）。build `9`は候補ソースの後に修正が十数コミット入った
  ため公開せず破棄した。D7は2026-08-31に毎リリースの必須ゲートから外れており（オーナー判断。実施
  条件は[manual-golden-paths.md](manual-golden-paths.md)）、この版では実施していない。
  **入場ms・クリックms・AXヒット率のunified logからの集計だけは未実施で、R14の外へ残る。**
...
- **印は常に1つだけ**（リング → AX枠 → 回答枠と入れ替わる。回答枠へ進む条件は「印の根拠」）。
  バブルは配置未確定の場合だけ回答枠に追従する。
...
| クリック | 質問（その場所／囲んだ範囲について） | **操作**。対象アプリへ通す。こちらは`leftMouseUp`を見て、画面が落ち着いたら撮って評価する。**文字入力欄へのクリックは撮らず、最初のキーから数えて入力が約0.8秒止まった時に撮る**（打たずに離れた欄は操作ではない。2026-09-06）。評価中の次のクリックは、撮影前なら吸収、**撮影後なら評価を打ち切って最新から**（`GuidanceTrigger`、2026-09-03） |
| スクロール | 覆いの上では届かない | **撮らない。** 枠だけが要素に追従する（AXの位置を読み直す。0.13ms）。対象の無い指示（言葉だけ）の時だけ、止まって0.6秒で撮り直す。バブルの上のスクロールは数えない |
| 印 | リング→AX枠→回答枠 | 次に押す対象の**枠（ビート付き）**。クリックで消え、評価後に次の対象へ。**スクロールに追従し、要素が消えれば消える** |
...
- 撮影の合図（`CopilotCaptureCuePresenter`）は当時は残したが、**2026-09-07に撤去**（幕の無い実画面では不具合に見えた）。
...
- **押した瞬間** ＝ 現在は合図なし。撮り直しのキュー（撮影範囲の幕が走査線1本分だけ戻る）は2026-09-07に撤去した
  （幕の無い画面では不具合に見えた）。R16で「押した」の合図が要るなら、ここを設計する。
...
### R17 — バブルの対話スレッド（完了、2026-09-03）

Visionのバブルは、最後の質問（紫のチップ）と最後の回答の1組だけを表示し、次の質問で上書きしていた。
バブルの中でAIとやり取りする体験は、ユーザーにとってはメッセンジャーのそれである — 解説があり、その下に
自分の発言、その下に回答。前のやり取りが消えるのは違和感で、自分の発言が一番上に出るのも逆である。

**不変条件**

```text
スレッドは1つの主題。主題が変わったら新しいスレッド。
見えているものは、モデルが覚えているものと一致する。
```

- **解説**の主題は「指した場所」。別の場所をクリック／囲めば主題が変わり、バブルは新しい対象の隣へ移り、
  スレッドは空から始まる（`VisionSession.point` の `turns = []` は従来どおり。**モデル契約・Gatewayは無変更**）。
  1つの主題の中で打った質問と回答は、下へ積まれる
- **案内**の主題は「目的」。案内中のクリックは操作であって主題の変更ではないので、一手ごとの指示は同じ
  スレッドへAIの発言として積まれる（進捗ターンは以前から `wireTurns` で会話全部を送っており、
  見えているものと送っているものが一致する）。「操作は検知しましたが、画面に変化は見えませんでした」の
  注記も、状態行ではなくスレッドの行として、それが修飾する一手の直前に出る
- **1回の覆い＝1セッション（履歴も記憶も通しで保持）** は検討して却下した。Web版でturnsを捨てない版は
  タップのたびに最初の回答が返った実績があり、`turns` は直近20件で古い方が黙って落ちるので、
  長いセッションでは「見えているのに覚えていない」状態が必ず起きる

**形**

- 行は上から順。AIの発言は左でアプリアイコン、ユーザーの発言は右で本人のアバター
  （Supabaseの `user_metadata.avatar_url` ／ `picture`。Google認証が供給する。無ければプレースホルダ）。
  ユーザー発言の地は製品の紫（`MarkStyle`）、AI発言の地は読む面のトーン（`BubbleSurface.reading`）。
  1つの回答を包んでいた読む面は無くなり、発言ごとの面になった
- クリック／囲みは「ここについて」「この範囲について」という短いユーザー発言としてスレッドの先頭に出る
  （それまでチップから隠していた `pointedHereText` をそのまま見せる。囲みは `pointedRegionText` を新設）
- 末尾の1行だけが「いま起きていること」を言う: エラー ＞ 書かれつつある回答 ＞ 待ち（「ここを読んでいます…」）。
  何も無ければ「画面のどこかをクリックすると…」のヒント
- 収まる時はそのまま、収まらない時はスクロールし、行が増えたら末尾へ追従する。高さの予算
  （下限10行・上限は画面の残り）は読む面からスレッド全体へ引き継いだ
- Composeは触っていない（入力補完であり対話ではない）。状態チップ（解説／案内＋×）と案内の状態行は現位置のまま

**最初の実機（2026-09-03、VS Codeで案内中）から直したもの**

- **同じ人に2つの顔が出た。** 行ごとに `AsyncImage` が別々に取得していたため、1行目は画像、2行目は
  プレースホルダになった（どちらの取得が済んでいたか次第）。**画像は `AuthViewModel.avatarImage` として
  URLごとに1回だけ読み、全行が同じ1枚を描く。** 読めていなければ全行がプレースホルダ
- **顔は1つの部品・1つの径。** AI側のアプリアイコンは角丸の四角、ユーザー側は丸、プレースホルダの
  SF Symbolは枠の内側に余白を持つ — 3つが別物に見えた。すべて同じ円にクリップし、**径は「1行の発言の高さ」
  （行高＋上下余白6×2＝約28pt、`VisionThreadView.avatarSize(fontSize:)`）**にした。1行の発言と顔の上下が揃い、
  長い発言は顔の下へ垂れる。プレースホルダは塗った円の中に `person.fill` を置いて描く
- **案内の状態行の説明文を削除した。** 「枠の場所をクリックしてください。操作後の画面を自動で確認します」は
  枠を見ている人に手順を説明しており、「待っています…」「確認しています…」も配管の説明だった。
  **進行中はスレッド末尾の「画面を読んでいます…」（解説と同じ言葉）**に一本化し、状態行には結果
  （到達した／撮影できなかった／確認が要る／上限）と「再確認」ボタンだけを残す
- **チップは「案内中」で、アイコンが脈打つ**（`symbolEffect(.pulse)`、Reduce Motionでは静止）。
  いまそのモードの中にいることを名乗る。「解説」はそのまま（動作ではなく製品の待機位置）

**実装**: 行の規則は `VisionThreadRow.rows`（純関数、`VisionThreadRowsTests` 9件で固定）、描画は
`VisionThreadView`、アバターは `ThreadAvatar`。`AuthViewModel.adoptAvatar(url:)` が `refreshState` で
URLと画像を埋め、サインアウトで消す。`VisionBubbleView` は `userAvatar: NSImage?` を注入で受ける
（テストは無アカウントで測る）。

**実機で確認済み**（2026-09-03）: 発言が下へ積まれ左右に分かれること、案内中に一手ごとの指示が積まれること、
別の場所を指すとスレッドが空から始まること。

**同日の見た目修正3件も実機で再確認済み**（2026-09-03）: 全行の顔が同じ1枚・同じ径、案内中の待ちが末尾の
「画面を読んでいます…」として出る、「案内中」の脈動。あわせて印の根拠の改訂（R14）と格子の重力撤去も確認した。
**R17は完了。** 案内の状態チップと状態行の見た目（R15で後回しにしていたもの）もここで課題として閉じた。
