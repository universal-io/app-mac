...
最終更新: 2026-08-24 ／ ステータス: `v0.2.2`（build `8`）を公開ダウンロードへ切替済み（D7通過）。`main`は`0.2.3`を開いている。R12はE1（測定）進行中、R14（実画面の上で指して聞くVision）は**F1〜F11実装済み・実機で動作中（指した対象の認知精度100%、2026-08-24）**、残りはF12（署名ビルドとD7相当）
...
- Copilotの開始導線は、ユーザーが質問を打ち込んだターンに出す（`VisionSession.offersGuidance`）。
  語句一致でも回答modeでも判定しない。指差しだけのターンには出さない — そのターンの本文
  「ここについて」がCopilotの目的になってしまう。
...
- **F5・F7・F9（実装完了・実機確認待ち）** 3段に分けず1つにした。F2で「バブルは覆いと同一
...
- **印は常に1つだけ**（リング → AX枠 → 回答枠と入れ替わる）。バブルも回答枠に追従する。
...
### Copilotの入口（2026-08-25に実装・クライアント側で完結）

**指差しでCopilotが起動しないのは正常である。** 指した時にユーザーが求めているのはそれについての
答えで、案内を始める意図は持っていない。**Copilotが起動するのはユーザーが質問した時**である。

```text
何かアクションを求めている質問なら、基本はCopilot。
出ないくらいなら、出やすい方に傾ける。ボタンは1つでよい。
```

**採った道: 打ち込んだ質問には常に出す。** `canStartCopilot`が使っていた
`isActionSeekingQuestion`のキーワード26語一致は削除した。実測で「Google Analyticsでどうすれば
国別のアクセス解析が見れるんですか?」「レポートを出力するには?」「請求書を発行したい」
「国別のアクセスを見たい」がすべて落ちており（「どうやって」はあるが「どうすれば」が無い）、
語を足す作業に終わりが無い上、**機構そのものが「出やすい方へ傾ける」と逆を向いていた**。

判定は`VisionSession.offersGuidance(turns:)`の純関数1つ:

- 回答（assistantターン）が存在すること
- 最後のユーザーターンが存在し、それが`pointedHereText`でないこと

この2条件は`startCopilot()`自身のguardと同じなので、**押せば必ず何かが起きる時だけボタンが出る**。
回答modeは見ない（`.guide`をORで残すと、指差しターンで目的が「ここについて」のままCopilotが
起動してしまう）。副作用として「これは何ですか?」にも入口が出るが、要望の傾きどおり受け入れる。
固定は`CopilotEntranceTests`（5件、上の実測表の文がそのまま入っている）。

**モデルに意図を返させる道（`api-gateway`のschema拡張）は採らなかった。** mainへのpushが本番
デプロイで、公開中のmacOSクライアントの挙動も同時に変わるため。必要になれば併用として足せる。

**起動した後は実機未確認である。** `startCopilot()`が覆いを閉じてパネル／ストリップへ渡す経路は
R14で一度も通っていない。構造は確認済み（`applyPanel`の`.navigator, .copilot`分岐が
`pointingOverlay.close()`してから`VisionRootView`＝ストリップだけを出す）。回路を開けたので、
次に見るのはそこ。
