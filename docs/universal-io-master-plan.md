...
5. **Copilotへの導線は現行を維持する。** 指差しは通常Visionの入口を置き換えるが、`guidance`ターンと
   Copilotストリップの構造は変えない。**→ R15で改める**（2026-08-25の実機で、覆いの上では案内が
   成立しないことが分かった。ストリップは撤去し、案内は同じセッションの2つ目の状態になる）。
...
### R15 — 実画面の上の案内（Copilotを覆いの上から実画面へ・設計、論点は承認待ち）
...
| キー | 覆いがキー（Esc・入力が効く） | **対象アプリのもの**。バブルの入力欄をクリックした時だけこちらがキーを取る |
...
- **どこからでも閉じる: 解説を閉じる／右Shift 2回**（`handleDoubleTap`は`.copilot`でも既に閉じる）。
  **Escは案内中は対象アプリへ行く**（こちらがキーでない）。入力欄にいる時だけ効く。したがって案内中の
  確実な出口は×とダブルタップで、「Esc」の表記は指す状態だけに出す。
...
**バブル上のクリックは進捗ターンを起こさない。** `addGlobalMonitorForEvents`は他アプリへ配送された
イベントだけを受け、自アプリのイベントは来ない（現ストリップも同じ性質で動いている）。
...
- **G1 probe**（短命・リポジトリ外か短命ブランチ）: 子パネル化の測定 — IME確定、Esc、キーの受け渡し、
  枠越しのクリックが対象アプリへ届くか、表示中の`ignoresMouseEvents`切替、案内に入った時点で対象アプリが
  前面か（F2で未決着のまま）。**機構の選択はここで決める。**
...
| | 論点 | 推奨 | 代替 |
|---|---|---|---|
| A | 案内への入口 | **`guide`回答で自動**（今日の実機が「ボタンは押されない」を示した） | ボタン維持 |
...
それを指差しとして飲んだ。何が起きたかと次の設計は [R15](#r15--実画面の上の案内copilotを覆いの上から実画面へ設計論点は承認待ち)。
