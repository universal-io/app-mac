...
最終更新: 2026-09-02 ／ ステータス: `v0.2.2`（build `8`）を公開ダウンロードへ切替済み（D7通過）。`main`は`0.2.3`を開いている。R14・R15は実機で動作中（指した対象の認知精度100%、2026-08-24／案内が成立、2026-08-25）で、R14のバブル配置を1ターン1配置の状態機械へ改修中。残りは実機確認とF12（署名ビルドとD7相当）。R12はE1（測定）進行中。**R16（道具が押す＝操作）は2026-08-30に設計を確定し、未着手。**
...
2. **配置の優先順位は、ユーザーの囲み／AX実測枠 → 最終検証済み回答の枠 → 配置不能。** ローカルで
   確定した幾何を、後着のモデル推定で上書きしない。クリック点は配置根拠にしない。
