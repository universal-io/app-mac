...
### Copilotの入口（次の実装テーマ・2026-08-25決定）

**指差しでCopilotが起動しないのは正常である。** 指した時にユーザーが求めているのはそれについての
答えで、案内を始める意図は持っていない。**Copilotが起動するのはユーザーが質問した時**であり、
アクションを求めている質問かどうかは判定できるはずのものである。
...
問題は判定機構である。`canStartCopilot`は`isActionSeekingQuestion`のキーワード26語一致で決めており、
**「どうやって」はあるが「どうすれば」が無い。** 実測で、「Google Analyticsでどうすれば国別の
アクセス解析が見れるんですか?」「レポートを出力するには?」「請求書を発行したい」がすべて落ちる。
語を足す作業に終わりが無く、**機構そのものが「出やすい方へ傾ける」と逆を向いている**。

道は3つ（詳細と実測表はHANDOFF）: モデルに意図を返させてゲートにする（`api-gateway`側の変更）、
打ち込んだ質問には常に出す（クライアントだけで完結）、併用。

**起動した後は実機未確認である。** `startCopilot()`が覆いを閉じてパネル／ストリップへ渡す経路は
R14で一度も通っていない。回路を開けたら次に見るのはそこ。
