...
最終更新: 2026-09-13 ／ ステータス: `v0.2.4`（build `11`、タグ`v0.2.4`）を公開ダウンロードへ切替済み。R14・R15は実機で動作中（指した対象の認知精度100%、2026-08-24／案内が成立、2026-08-25）。2026-09-06にオーナーの実機で壊れた案内ループは7件の欠陥を直して2026-09-08に実機で通り、**その修正を含む署名ビルドが公開版になった**（スモーク 2026-09-13）。**F12は閉じ、R14は完了。** R12はE1（測定）進行中。**R16（道具が押す＝操作）は2026-08-30に設計を確定し、未着手。** **R17（バブルの対話スレッド）は2026-09-03に完了**（実装・同日の見た目修正・印の根拠の改訂・幕の重力撤去まで実機確認済み）。221 unit test通過。**次はR8 M6（Skillsの拡充）。**
