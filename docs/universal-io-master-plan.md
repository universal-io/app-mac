...
最終更新: 2026-09-03 ／ ステータス: `v0.2.3`（build `10`、タグ`v0.2.3`）を公開ダウンロードへ切替済み。R14・R15は実機で動作中（指した対象の認知精度100%、2026-08-24／案内が成立、2026-08-25）。R14のバブル配置は1ターン1配置の状態機械へ改修し、印とカードを独立させた挙動まで実機確認済み（2026-09-02）。**F12は閉じ、R14は完了。** Composeのバブルも同じ署名ビルドで実機確認済み（2026-09-03）。R12はE1（測定）進行中。**R16（道具が押す＝操作）は2026-08-30に設計を確定し、未着手。** R17（バブルの対話スレッド）は2026-09-03に実装し、同日の実機で挙動を確認。同日の見た目修正・印の根拠の改訂・
幕の重力撤去が再確認待ち。199 unit test通過。
...
### R17 — バブルの対話スレッド（実装済み・実機で挙動確認済み、2026-09-03）
...
**再確認が要るもの**: ①全行の顔が同じ1枚・同じ径・1行の発言と上下が揃うか（Googleアカウント／
マジックリンクのプレースホルダ）②案内中の待ちが「画面を読んでいます…」として末尾に出るか
③「案内中」の脈動が強すぎないか ④長いスレッドで上限に当たった時のスクロールと、空き面でのドラッグ。
