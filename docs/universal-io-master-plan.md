...
- **どこからでも閉じる: 解説を閉じる／右Shift 2回**（`handleDoubleTap`は`.copilot`でも既に閉じる）。
  **案内中のEscは、その時どちらがキーかで行き先が変わる**（実機で確認、2026-08-25）。案内に入っても
  バブルはキーを持ったままなので、**Escは普通こちらに届いてセッションを閉じる**。対象アプリを
  クリックした後はそちらがキーなので、Escはアプリへ行く。設計時に「案内中のEscは対象アプリのもの」と
  書いたのは誤りで、キーを手放す処理を入れていない。**同じキーが状況で2つの意味を持つので、
  確実な出口は×とダブルタップとし、「Esc」の表記は解説の時だけ出す。**
