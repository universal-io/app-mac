...
**指を置いた瞬間に撮る。** 入場時の1枚を指差しの間ずっと使い回す案は、F2の実測で棄却した。覆いを
出すと自アプリが前面を取り、**対象アプリが非活性になって画面が変わる**（フォーカスリング、選択色、
開いていたネイティブメニュー）。使い回すと、ユーザーは変わった後の画面の位置Pをクリックし、
こちらは変わる前の画像の位置Pへマークを焼く — **別のものについての回答が返る**。撮影1回を節約する
代わりに対象を間違える取引は成立しない。Web版の規則（`solo-mode.md` §1「ジェスチャーが始まった
瞬間、そこにあったものが対象」）がそのまま正しい。
...
- **F2（完了・リポジトリ外の短命probeで実測）** 2つの未確認が決まった。
  - **非活性のままは無理。** `.nonactivatingPanel`＋`orderFrontRegardless`でも、覆いを出した時点で
    `NSApp.isActive=true`になり前面を取る（ユーザー操作の前、起動0.145秒後）。したがって対象アプリは
    非活性になる。これが上の「設計の芯」を変えた。
  ...
  - **予想と逆の副産物。`.nonactivatingPanel`＋`makeKey()`は同期的にキーを取り、
    `NSApp.activate`＋`makeKeyAndOrderFront`は出した直後まだキーになっていない**
    （macOS 14の協調的アクティベーションが非同期）。activationを避けられないとしても、
    最初のクリックが即座に効くべき製品なので構成は前者を採る。既存の`ScreenshotSelectionOverlay`は
    後者の形である。
...
1. **こちらが前面を取ると、対象アプリの開いたネイティブメニューは閉じるか。** R12が扱う
   「開いたプルダウンを説明する」がこの条件そのものである。ChromeのHTMLプルダウンはDOMなので
   閉じないはずだが、`NSMenu`は閉じる。測り方: probeに「起動からN秒待って覆いを出す」を足し、
   その間にメニューを開く。閉じるなら、メニューが開いている画面はクリック時撮影では救えず、
   入場時の撮影に頼るしかない領域として別に扱う。
2. **撮影1回の所要ms**（F3で記録を足して測る）。毎クリック撮り直しが体感に耐えるか、
