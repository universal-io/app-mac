...
### Copilotの入口（未着手・2026-08-25に原因特定）

**指差し・囲みのターンでは「案内を開始」が出ない。** `canStartCopilot`が最後のユーザーターンの
テキストを`isActionSeekingQuestion`に通すが、指差しのターンではそれが固定文字列「ここについて」で、
どのマーカーにも一致しない。モデルが`mode == .guide`を返さない限りボタンは現れない。

R14の主要操作には質問文が無いのだから、**質問文の語彙でゲートを作る前提そのものが成立していない**。
ターンの種類と回答の内容で決め直すか、案内への入口を質問とは独立に常設する。
「直接アクションを間接シグナルで拒否しない」という既定方針に、マーカー一致は違反している。
