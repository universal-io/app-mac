...
最終更新: 2026-08-04 ／ ステータス: `0.2.2` build `6`をテスター向け候補として配布中（公開ダウンロードは`v0.2.1`のまま。D7未達）。R12はE1（測定）進行中
...
L4（AX 2パス目）→ L5（Copilot撮影予算＝R12 E2と合流）→ L6（review/suggest逐次化）→
...
足り、下の判定は後者にだけ適用する。2026-08-04時点で`0.2.2` build `6`は前者まで実施済み、
公開ダウンロードは`v0.2.1`のままである（D7未達）。
