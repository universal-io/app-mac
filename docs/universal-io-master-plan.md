...
最終更新: 2026-08-30 ／ ステータス: `v0.2.2`（build `8`）を公開ダウンロードへ切替済み（D7通過）。`main`は`0.2.3`を開いている。R14・R15は実機で動作中（指した対象の認知精度100%、2026-08-24／案内が成立、2026-08-25）で、残りはF12（署名ビルドとD7相当）。R12はE1（測定）進行中。**R16（道具が押す＝操作）は2026-08-30に設計を確定し、未着手。**「## 製品」の一文を「操作」が成立する形へ書き換えたのも同日
