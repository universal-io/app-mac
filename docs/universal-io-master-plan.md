...
| クリック | 質問（その場所／囲んだ範囲について） | **操作**。対象アプリへ通す。こちらは`leftMouseUp`を見て、画面が落ち着いたら撮って評価する。**文字入力欄へのクリックは撮らず、入力が約0.8秒止まるまで待つ**。評価中の次のクリックは、撮影前なら吸収、**撮影後なら評価を打ち切って最新から**（`GuidanceTrigger`、2026-09-03） |
| スクロール | 覆いの上では届かない | **撮らない。** 枠だけが要素に追従する（AXの位置を読み直す。0.13ms）。対象の無い指示（言葉だけ）の時だけ、止まって0.6秒で撮り直す |
